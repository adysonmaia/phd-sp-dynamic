...
def data_analysis(scenario, valid_load_filename, predicted_load_filename):
    vl_df = pd.read_json(valid_load_filename, orient='records')
    vl_df['time'] = pd.to_datetime(vl_df['time'], unit='s', utc=True)
    vl_df.set_index(['time'], inplace=True)
    vl_df.sort_index(inplace=True)

    pl_df = pd.read_json(predicted_load_filename, orient='records')
    pl_df['time'] = pd.to_datetime(pl_df['time'], unit='s', utc=True)
    pl_df['prediction_time'] = pd.to_datetime(pl_df['prediction_time'], unit='s', utc=True)
    pl_df.set_index(['time'], inplace=True)
    pl_df.sort_index(inplace=True)

    accuracy_metrics = ['mae', 'rmse', 'smape', 'umbrae']
    accuracy_data = []
    time_series = vl_df.index.unique().tolist()
    time_series = time_series[1:]
    for t in time_series:
        print(t.astimezone(SF_TZ))
        l_df = pl_df.loc[t].set_index(['prediction_time', 'app', 'node']).sort_index()
        l_df.index.names = ['time', 'app', 'node']
        # print(l_df)

        r_df = vl_df.set_index(['app', 'node'], append=True).sort_index()
        join_df = l_df.join(r_df, how='inner')
        # join_df = join_df.groupby(level='time').sum()
